...
if(isset($_POST['barcode']) && $_POST['barcode']!=""){
	$startement = $db->prepare("SELECT * FROM invoice join customer ON invoice.customer_id=customer.customer_id WHERE barcode LIKE :barcode");

	$startement->execute(array(
		'barcode'=>'%'.$_POST['barcode'].'%'
		));
	if($startement->rowCount()==0){
		echo '<h1 style="color:red;text-align:center;">no invoice was found !!</h1>';
	}else{
